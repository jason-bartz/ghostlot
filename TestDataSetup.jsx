...
import { AlertCircle, Check, Loader, RefreshCw, Database, Car, QrCode, BarChart2, User, Trash } from 'lucide-react';
import dataImportService from './DataImportService';

// This component connects to the DataImportService to generate and manage test data
...
const TestDataSetup = () => {
  ...
  const generateTestData = async () => {
    ...
    try {
      ...
      setStatus({ message: `Error generating test data: ${error.message}`, type: 'error' });
    } finally {
  ...
  const clearTestData = async () => {
    ...
    try {
      ...
      setStatus({ message: `Error clearing test data: ${error.message}`, type: 'error' });
    } finally {
      setIsClearing(false);
    }
  };
...
// Missing Calendar and BookOpen icons
